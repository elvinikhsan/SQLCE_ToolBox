...
BEGIN  
		...
		SET @sqltext = 'USE ' + QUOTENAME(@dbname) + ';
						SELECT DISTINCT ''' + REPLACE(@dbname, CHAR(39), CHAR(95)) + ''' AS [DatabaseName], schema_name(so.schema_id) AS SchemaName, object_name(so.object_id) AS TableName, max(dmv.rows) AS ApproximateRows, MAX(d.ColumnCount) AS ColumnCount
...
BEGIN
	...
	INSERT INTO @tblNoIndex
	EXEC (@sqlcmd);
	SET @j=@j+1;
END

SELECT * FROM @tblNoIndex ORDER BY ID;
